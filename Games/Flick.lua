...
do
    ...
    statusGroup:AddLabel("Version: " .. (getgenv().unxshared and getgenv().unxshared.version or "Unknown"))
                ...
                while autoRespawnEnabled do
                    if not workspace:FindFirstChild(LocalPlayer.Name) then
                        if tick() - autoRespawnLastFire >= 1 then
                            ...
                            if remotes and remotes:FindFirstChild("Command") then remotes.Command:FireServer("Play") end
    ...
    otherGroup:AddSlider("AutoRespawnDelay", { Text = "Auto Respawn Delay", Default = 0, Min = 0, Max = 3, Rounding = 2, Suffix = "s", Callback = function(v) autoRespawnDelay = v end })
    otherGroup:AddDivider()
    otherGroup:AddButton("Lobby", function() 
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        if remotes and remotes:FindFirstChild("Command") then remotes.Command:FireServer("Lobby") end 
    end)
    otherGroup:AddButton("Play", function() 
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        if remotes and remotes:FindFirstChild("Command") then remotes.Command:FireServer("Play") end 
    end)
...
do
    ...
    esptab:AddCheckbox("Tracers", { Text = "Tracers", Default = false, Callback = function(v) 
        tracersenabled = v 
        if v then
            createtracers()
    ...
    end }):AddColorPicker("TracerColor", { Default = Color3.fromRGB(255,255,255), Title = "Tracer Color", Callback = function(v) espconfig.tracercolor = v end })
...
do
    ...
    local AimWeapon, AimStateChanged, FireWeaponMobile, Sound_Request, CheckFire, CheckShot, ProjectileRender, ProjectileFinished
    task.spawn(function()
        ...
        if clientRemotes then
            CheckFire = clientRemotes:WaitForChild("CheckFire", 10)
            CheckShot = clientRemotes:WaitForChild("CheckShot", 10)
        end
        local gunModules = ReplicatedStorage:FindFirstChild("ModuleScripts")
        if gunModules then gunModules = gunModules:FindFirstChild("GunModules") if gunModules then gunModules = gunModules:FindFirstChild("Remote") if gunModules then ProjectileRender = gunModules:FindFirstChild("ProjectileRender") ProjectileFinished = gunModules:FindFirstChild("ProjectileFinished") end end end
    end)
...
do 
    ...
    featuresGroup:AddDivider()
    featuresGroup:AddToggle("BulletTrails", {
        Text = "Show Bullet Trail",
        Default = false,
        Callback = function(v)
            bulletTrailsEnabled = v
        end
    }):AddColorPicker("MissColor", {
...
do
    ...
    funGroup:AddSlider("RGBSpeed", { Text = "RGB Speed (Local)", Default = 10, Min = 1, Max = 50, Rounding = 0, Callback = function(v) rgbSpeed = v end })
...
task.spawn(function()
    ...
    while true do
        ...
        if count >= 5 then continue end

        local ls = LocalPlayer:FindFirstChild("leaderstats"); if not ls then continue end
        local k = ls:FindFirstChild("Kills"); if not k then continue end

        local m = math.floor(k.Value / 50)
        if m > last and m > 0 then
            last = m
            count += 1
            ...
            Library:Notify({
                Title = "SLOW DOWN! IF YOU KILL TOO MANY PLAYERS IN A SHORT PERIOD OF TIME, YOUR ACCOUNT MIGHT BE BANNED!",
                Duration = 5
            })
        end
    end
