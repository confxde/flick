...
print("")
print("oracle | Anything Below Here Is Probably Oracle Related Errors!")

if not isfolder("oracle") then makefolder("oracle") end
if not isfolder("oracle/themes") then makefolder("oracle/themes") end
if not isfile("oracle/themes/default.txt") then writefile("oracle/themes/default.txt", "oracle") end
if not isfile("oracle/themes/oracle.json") then writefile("oracle/themes/oracle.json", '{"MainColor":"131218","FontFace":"Code","AccentColor":"a970ff","OutlineColor":"262434","BackgroundColor":"0b0b0d","FontColor":"e8e6f2"}') end
...
elseif not isfile("AlreadyRated.unx") then
    ...
    "https://raw.githubusercontent.com/oracle/flick/refs/heads/main/Modules/v2/Rating.lua"
    ))()
...
s("Oracle Loader v2.1.0 initialized")
s("[WARNING]: By executing Oracle you accept our Terms Of Service")
s("[WARNING]: Learn more on http://getunx.vercel.app/tos.html")
s("[WARNING]: PLEASE READ THE MESSAGE ABOVE!!!!!")
s("Checking API Status...")
loadstring(game:HttpGet("https://raw.githubusercontent.com/oracle/flick/refs/heads/main/Modules/v2/API.lua",true))()
task.wait(0.1)
s("Creating global variables...")
task.wait(0.05)
getgenv().unxshared={version="2.3.2 (Patch 1 [Release 2])",gamename=az:GetProductInfo(game.PlaceId).Name,issupported=false,playername=d.Name,playerid=d.UserId,isloaded=false,devnote="Made with 💖 By Confxde"}
...
task.wait(0.12)
local bd={[12240122896]="https://apigetunx.vercel.app/Games/FigureL.lua",[136801880565837]="https://apigetunx.vercel.app/Games/Flick.lua",[893973440]="https://github.com/oracle/flick/raw/refs/heads/main/Games/Flee.lua"}
local be,bf
...
if bd[bc]then
    ...
    task.wait(0.1)
    be,bf=pcall(function()
        local scriptUrl="https://raw.githubusercontent.com/oracle/flick/refs/heads/main/Games/Universal.lua"
        local src=game:HttpGet(scriptUrl)
        local func,err=loadstring(src,scriptUrl)
        if not func then error(err) end
        func()
...
if be then
    ...
    loadstring(game:HttpGet("https://github.com/oracle/flick/raw/refs/heads/main/Modules/v2/Log.lua",true))()
    task.wait(0.05)
    ...
    "<font color='rgb(0,170,255)'>https://discord.gg/wJDjjZHDk5</font>"
